...
// ----------------- helpers -----------------
const parseLooseDate = (s) => new Date(String(s).replace(/(\d):(\d)(?!\d)/g, '$1:$2')) // tolerates 20:6:52
const fmtDate = (s) =>
  parseLooseDate(s).toLocaleDateString(undefined, { year: 'numeric', month: 'short', day: 'numeric' })
const fmtTime = (s) =>
  parseLooseDate(s).toLocaleTimeString(undefined, { hour: '2-digit', minute: '2-digit' })
...
const PLANET_COLORS = {
  Sun: '#fbbf24', Moon: '#a78bfa', Mercury: '#10b981', Venus: '#ec4899',
  Mars: '#ef4444', Jupiter: '#3b82f6', Saturn: '#6366f1', Rahu: '#8b5cf6'
}
...
const getNextFutureTransit = (list) => {
  const items = (list || []).map(t => ({ ...t, _time: parseLooseDate(t.date).getTime() }))
  const now = Date.now()
...
endDate: new Date(endItem._time).toLocaleString(undefined, {
  ...
  const ORDER = ['Moon', 'Mercury', 'Venus', 'Sun', 'Mars', 'Jupiter', 'Saturn', 'Rahu']
...
const getTimeframe = () => {
  const frames = ['Next 30 days', 'Next 3 months', 'Next 6 months', 'Current phase (2-3 weeks)']
...
const filteredTransits = useMemo(() => {
  ...
  return all.slice(1).filter(t => parseLooseDate(t.date).getTime() <= limit)
  ...
  const nextTwo = useMemo(() => {
    ...
    Object.entries(result.upcomingTransits).forEach(([planet, list]) => {
      ...
      const next = list
        .map(t => ({ ...t, _time: parseLooseDate(t.date).getTime() }))
        .find(t => t._time >= now)
      if (next) picks.push({ planet, sign: next.sign, date: next.date, _time: next._time })
    })
    ...
    <div className="transit-container">
      ...
      <div className="transit-content">
        <header className="transit-header">
          ...
          <p className="subtitle">Current planetary movements and their cosmic influences</p>
        </header>
            ...
            <section className="transit-section">
              ...
              <div className="countdown-grid-cards">
                {nextTwo.map((item, idx) => {
                  ...
                  const current = result.currentTransits.find(t => t.planet === item.planet)
                  return (
                    <div key={`${item.planet}-${idx}`} className="transit-card countdown-card-new current">
                      <div className="accent" style={{ background: `linear-gradient(90deg, ${color}, ${color}dd)` }}></div>

                      <div className="cardBody">
                        <div className="iconBox" style={{ background: `linear-gradient(135deg, ${color}, ${color}cc)` }}>
                          <span style={{ fontSize: '2rem' }}>{ZODIAC_SYMBOLS[item.sign] || '✦'}</span>
                        </div>
                        <div className="titleGroup" style={{ flex: 1 }}>
                          <h3>{item.planet} enters {item.sign}</h3>
                      ...
                      <div className="cardBody info-section">
                        <div className="infoRow">
                          ...
                          <span className="infoValue">{current?.currentSign || '—'}</span>
                        </div>
                        <div className="infoRow">
                          <span className="infoLabel">Next Transit</span>
                          <span className="infoValue">{fmtDate(item.date)} • {fmtTime(item.date)}</span>
                        </div>
                        <div className="infoRow">
            ...
            <section className="transit-section">
              ...
              <div style={{
                ...
                {result.currentTransits.map((transit, index) => {
                  const planetColor = PLANET_COLORS[transit.planet] || '#d4af37'
                  return (
                    <div key={index} className="transit-card current-transit-card">
                      ...
                      <div className="cardBody info-section">
                        <div className="infoRow"><span className="infoLabel">Duration</span><span className="infoValue">{transit.duration}</span></div>
                        <div className="infoRow"><span className="infoLabel">Started</span><span className="infoValue">{transit.startDate}</span></div>
                        <div className="infoRow"><span className="infoLabel">Ends</span><span className="infoValue">{transit.endDate}</span></div>
                      </div>
                      <div className="footer">Next: {transit.nextTransit?.sign || '—'}</div>
                    </div>
                  )
                })}
              </div>
...
<section className="transit-section">
  ...
  <div className="transit-table-card">
    <div className="planet-tabs-container">
      ...
      <div className="timeframe-filters" style={{ marginTop: '1rem', display: 'flex', gap: '1rem', justifyContent: 'center' }}>
        <button
          className={`filter-btn ${selectedRange === '6m' ? 'active' : ''}`}
          onClick={() => setSelectedRange('6m')}
        >
          Next 6 Months
        </button>
        <button
          className={`filter-btn ${selectedRange === '5y' ? 'active' : ''}`}
    ...
    <div className="upcoming-transits-section">
      <h3 className="upcoming-section-title">
        {selectedPlanet} Transits Schedule ({selectedRange === '6m' ? 'Next 6 Months' : 'Next 5 Years'})
      </h3>

      {filteredTransits.length > 0 ? (
        <div className="upcoming-transit-grid">
          {filteredTransits.map((transit, index) => {
            const planetColor = PLANET_COLORS[selectedPlanet] || '#d4af37'
            return (
              <div key={index} className="transit-card upcoming-card">
                <div className="accent" style={{ background: `linear-gradient(90deg, ${planetColor}, ${planetColor}dd)` }}></div>
                <div className="cardBody">
                  <div className="iconBox" style={{ background: `linear-gradient(135deg, ${planetColor}, ${planetColor}cc)` }}>
                    <span style={{ fontSize: '1.5rem' }}>{ZODIAC_SYMBOLS[transit.sign] || '✦'}</span>
                  </div>
                  <div className="titleGroup" style={{ flex: 1 }}>
                    <h3>{transit.sign}</h3>
                ...
                <div className="cardBody info-section">
                  <div className="infoRow">
                    ...
                    <span className="infoValue">{fmtDate(transit.date)}</span>
                  </div>
                  <div className="infoRow">
                    <span className="infoLabel">Time</span>
                    <span className="infoValue">{fmtTime(transit.date)}</span>
                  </div>
                </div>
              </div>
            )
        ...
        <div className="empty-state">
          <div className="empty-state-card">
            <span className="empty-state-text">No upcoming transits within this timeframe.</span>
          </div>
        </div>
      )}
    </div>
