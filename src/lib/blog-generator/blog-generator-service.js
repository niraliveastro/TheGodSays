...
async function createBlogPost(keyword, generatedContent, internalLinks = []) {
  try {
    ...
    console.log(`✅ Created blog: "${generatedContent.title}" (ID: ${docRef.id})`)
...
export async function generateBlogs(options = {}) {
  ...
  try {
    ...
    console.log(`📝 Processing ${keywordsToProcess.length} blogs...`)
    ...
    for (let i = 0; i < keywordsToProcess.length; i++) {
      ...
      try {
        ...
        console.error(`  ❌ Error generating blog for "${keyword.title}":`, error)
        results.errors.push({
