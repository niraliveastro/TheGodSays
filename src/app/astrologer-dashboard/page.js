...
function AstrologerDashboardContent() {
  ...
  useEffect(() => {
    ...
    // Polling fallback mechanism for when SSE doesn't work reliably in production
    const pollInterval = setInterval(() => {
       if (sseStatus !== 'connected' || Date.now() - lastPollTime > 20000) { // Poll every 20 seconds or when SSE is not connected
         console.log('Polling for new calls (SSE status:', sseStatus, ')')
         fetchCalls()
         fetchQueue()
   ...
   }, [astrologerId, sseStatus, lastPollTime])
