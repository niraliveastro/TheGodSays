...
export default function VoiceCallRoom() {
  ...
  const handleDisconnect = async () => {
    try {
      ...
      console.log(
        `Ending voice call ${callId} with duration ${durationMinutes} minutes`
      );
      ...
      if (callId && durationMinutes > 0) {
        ...
        if (billingResult.success) {
          console.log(
            `✅ Voice billing finalized: ₹${billingResult.finalAmount} charged, ₹${billingResult.refundAmount} refunded`
          );
          ...
          console.error(
            "❌ Voice billing finalization failed:",
            billingResult.error
          );
