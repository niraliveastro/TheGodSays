...
import { getPublishedBlogsHumanWritten } from '@/lib/blog'
...
export async function generateMetadata() {
  const blogs = await getPublishedBlogsHumanWritten()
...
export default async function BlogPage({ searchParams }) {
  // Fetch only human-written blogs (AI-generated are on /zodiac-today)
  const allBlogs = await getPublishedBlogsHumanWritten()
      ...
      <div className="blog-container">
        {blogs.length === 0 ? (
          ...
          <>
            ...
            {/* Client component for real-time updates (human-written only; AI blogs on /zodiac-today) */}
            <BlogListClient initialBlogs={blogs} autoGeneratedOnly={false} />
          </>
        )}
      </div>
