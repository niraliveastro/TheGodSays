...
import { User, Calendar, MapPin, Clock, Star, Info, AlertCircle, ChevronDown } from 'lucide-react';
  ...
  return (
    <>
      <style jsx>{`
        ...
        @media (min-width: 640px) { .app { padding: 2rem 1.5rem; } }
        @media (min-width: 768px) { .app { max-width: 720px; } }
        @media (min-width: 1024px) { .app { max-width: 960px; } }
        @media (min-width: 1280px) { .app { max-width: 1200px; } }
        ...
        .cardTitle {
          ...
          font-family: 'Cormorant Garamond', serif;
          font-size: 1.25rem; font-weight: 700; color: #7c2d12;
        }
        ...
        input, select {
          ...
          font-size: .925rem; transition: all .2s;
        }
        ...
        .btn {
          ...
          padding: .75rem 1.5rem; background: #fff; border: 1.5px solid #e5e7eb;
          border-radius: .875rem; font-weight: 600; transition: all .3s; cursor: pointer;
        }
        ...
        .btn-primary:hover { background: #e6c04a; transform: translateY(-2px); }
        ...
        .profile-item {
          ...
          background: #fefce8; border: 1px solid rgba(212,175,55,.3);
        }
        .profile-label { font-size: .875rem; color: #7c2d12; margin-bottom: .25rem; }
        .profile-value { font-size: 1.25rem; font-weight: 700; color: #b8972e; }

        .dasha-row {
          display: flex; justify-content: space-between; padding: 1rem;
          background: #f9fafb; border-radius: .75rem; border: 1px solid #e5e7eb;
        }
        .dasha-label { font-weight: 600; color: #444; }
        .dasha-value { font-weight: 700; color: #b8972e; }

        .note-item {
          display: flex; align-items: flex-start; gap: .75rem; padding: .75rem; margin:10px;
          background: #fefce8; border-radius: .75rem; border-left: 4px solid #d4af37;
        }
        .note-dot { width: 8px; height: 8px; background: #d4af37; border-radius: 50%; margin-top: .5rem; flex-shrink: 0; }

        .hora-item {
          display: flex; justify-content: space-between; padding: 1rem; margin: 10px;
          background: #ecfdf5; border-radius: .75rem; border: 1px solid #bbf7d0;
        }
        .hora-time { font-size: .875rem; color: #065f46; }
        .hora-reason { font-size: .875rem; color: #15803d; text-align: right; }

        .section { margin-bottom: 2rem; }
        .sectionTitle {
          display: flex; align-items: center; gap: .5rem; margin-bottom: 1rem;
          font-family: 'Cormorant Garamond', serif; font-size: 1.25rem; color: #7c2d12;
        }

        .error-alert {
          display: flex; align-items: center; gap: .5rem; padding: 1rem;
          background: #fef2f2; border: 1px solid #fecaca; border-radius: .875rem;
          color: #b91c1c; font-size: .9rem; margin-top: 1rem;
        }
      `}</style>
        ...
        {!isSubmitted ? (
          <>
            <header className="header">
              ...
              <p className="subtitle">Enter your birth details for personalized insights</p>
            </header>

            <div className="card">
              <div className="cardHeader">
                <div className="cardTitle"><User style={{ width: 20, height: 20 }} /> Birth Details</div>
              </div>
              <div className="cardBody">
                <form onSubmit={handleSubmit} className="space-y-4">
                  ...
                  <div>
                    ...
                    <input name="birthTime" type="time" value={formData.birthTime} onChange={handleInputChange} required />
                  </div>
                  <div>
                    <label>Place of Birth</label>
                    <input name="birthPlace" value={formData.birthPlace} onChange={handleInputChange} placeholder="City, State, Country" required />
                  </div>
                  <button type="submit" className="btn btn-primary w-full">Get Personalized Panchang</button>
                </form>
              </div>
            </div>
          </>
          ...
          <>
            <header className="header">
              ...
              <p className="subtitle">Welcome, {personalizedData.userProfile.name}</p>
            </header>

            {/* Astrological Profile */}
            <div className="card section">
              <div className="cardHeader">
                <div className="cardTitle"><Star style={{ width: 20, height: 20 }} /> Your Astrological Profile</div>
              </div>
              <div className="cardBody">
                <div className="profile-grid">
                  <div className="profile-item">
                    <div className="profile-label">Lagna</div>
                    <div className="profile-value">{personalizedData.userProfile.lagna}</div>
                  </div>
                  <div className="profile-item">
                    <div className="profile-label">Moon Sign</div>
                    <div className="profile-value">{personalizedData.userProfile.moonSign}</div>
                  </div>
                  <div className="profile-item">
                    <div className="profile-label">Nakshatra</div>
                    <div className="profile-value">{personalizedData.userProfile.nakshatra}</div>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="card section">
              <div className="cardHeader">
                <div className="cardTitle"><Calendar style={{ width: 20, height: 20 }} /> Current Dasha Period</div>
              </div>
              <div className="cardBody">
                <div className="dasha-row">
                  <div>
                    <div className="dasha-label">Mahadasha</div>
                    <div className="dasha-value">{personalizedData.dasha.mahadasha}</div>
                  </div>
                  <div>
                    <div className="dasha-label">Antardasha</div>
                    <div className="dasha-value">{personalizedData.dasha.antardasha}</div>
                  </div>
                </div>
                <div style={{ marginTop: '1rem', padding: '1rem', background: '#fefce8', borderRadius: '.75rem' }}>
                  <div style={{ display: 'flex', alignItems: 'flex-start', gap: '.5rem' }}>
                    <Info style={{ width: 20, height: 20, color: '#d4af37', flexShrink: 0 }} />
                    <div>
                      <div style={{ fontWeight: 600, color: '#7c2d12' }}>Meaning</div>
                      <div style={{ color: '#555', fontSize: '.9rem' }}>{personalizedData.dasha.meaning}</div>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className="cardBody">
                {personalizedData.recommendedHoras.map((hora, i) => (
                  <div key={i} className="hora-item">
                    <div>
                      <div style={{ fontWeight: 600, color: '#166534' }}>{hora.planet}</div>
                      <div className="hora-time">{hora.start} - {hora.end}</div>
                    </div>
            ...
            <div className="section">
              ...
              <div className="card">
                <div className="cardBody">
                  <AstrologyOptions onOptionSelect={handleAstrologyOptionsSubmit} isLoading={isLoadingAstrology} />
                  {astrologyErrors && (
                  ...
                  {astrologyResults && (
                    <div style={{ marginTop: '1.5rem' }}>
                      <AstrologyResults results={astrologyResults} selectedOptions={selectedAstrologyOptions} />
                    </div>
                  )}
                </div>
              </div>
            ...
            <div className="section">
              ...
              <div className="card">
                <div className="cardBody">
                  <ChoghadiyaForm onSubmit={handleChoghadiyaSubmit} isLoading={isLoadingChoghadiya} />
                  {choghadiyaError && (
                    <div className="error-alert">
                      <AlertCircle style={{ width: 20, height: 20 }} />
                      {choghadiyaError}
