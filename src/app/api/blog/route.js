...
export async function GET(request) {
  try {
    ...
    const autoGeneratedParam = searchParams.get('autoGenerated') // 'true' = AI-only (zodiac-today), 'false' = human-only (blog)
    ...
    // Filter by autoGenerated for public listing pages (zodiac-today = AI only, blog = human only)
    let filteredBlogs = blogs
    if (status === 'published' && autoGeneratedParam !== null && autoGeneratedParam !== undefined) {
      if (autoGeneratedParam === 'true') {
        filteredBlogs = blogs.filter((b) => b.autoGenerated === true)
      } else if (autoGeneratedParam === 'false') {
        filteredBlogs = blogs.filter((b) => b.autoGenerated !== true)
      }
    }
    ...
    console.log(`[Blog API] Returning ${filteredBlogs.length} ${status || 'all'} blog(s)`)

    const blogsToReturn = filteredBlogs

    return Response.json({ blogs: blogsToReturn }, { 
      status: 200,
      headers: {
