...
export default function Home() {
  ...
  const fetchRealPanchangData = async () => {
    ...
    try {
      ...
      // Add retry for auspicious data since it's failing most often
      let panchangSettled, sunMoonSettled, auspiciousSettled

      try {
        [panchangSettled, sunMoonSettled, auspiciousSettled] = await Promise.allSettled([
        ...
        // If Promise.allSettled itself fails, create rejected promises
        panchangSettled = { status: 'rejected', reason: error }
        sunMoonSettled = { status: 'rejected', reason: error }
        auspiciousSettled = { status: 'rejected', reason: error }
      }
      ...
      if (hasAuthError) {
        ...
        setPanchangError('Using sample data due to API authentication issues. The astrology API service may require a valid API key.')
        setIsLoadingPanchang(false)
        return
      }

      // Check if all APIs failed with 500 errors (service unavailable)
      const all500Errors = [panchangSettled, sunMoonSettled, auspiciousSettled].every(result =>
        result.status === 'rejected' && result.reason?.message?.includes('500')
      )

      if (all500Errors) {
        console.warn('All astrology APIs are temporarily unavailable (500 errors), using mock data')
        setPanchangData(mockPanchangData)
        setPanchangError('Real-time astrology data is temporarily unavailable. Using sample data.')
        setIsLoadingPanchang(false)
        return
      }
      ...
      try {
        ...
        if (panchangResults.errors) {
          console.warn('[Panchang] API errors:', panchangResults.errors)
        }
        if (auspiciousData.errors) {
          console.warn('[Auspicious] API errors:', auspiciousData.errors)
        }
